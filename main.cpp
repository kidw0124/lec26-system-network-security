	      ...
	      struct nfq_data *nfa, void *data)
{
	...
	if(len>=0){
		...
		if(httpdata.size()>0&&
				(httpdata.find("GET")!=string::npos||httpdata.find("POST")!=string::npos||httpdata.find("PUT")!=string::npos||
				httpdata.find("HEAD")!=string::npos||httpdata.find("DELETE")!=string::npos||httpdata.find("CONNECT")!=string::npos||
				httpdata.find("OPTIONS")!=string::npos||httpdata.find("TRACE")!=string::npos||httpdata.find("PATCH")!=string::npos)
				/// header methods in https://developer.mozilla.org/ko/docs/Web/HTTP/Methods
			){
			if(httpdata.find(host)!=string::npos){
				printf("Blocked %s!\n",host.c_str());
